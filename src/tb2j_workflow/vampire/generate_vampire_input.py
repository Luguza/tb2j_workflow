...
# and the magnetisation of the whole system last (~0 for an antiferromagnet).
#------------------------------------------
output:temperature
...
def read_unitcell(ucf_path: Path) -> tuple[list[float], int, int]:
    ...
    def line_after(marker: str) -> str:
        ...
        raise TB2JError(f"{ucf_path} has no '{marker}' section, so it is not a Vampire unit cell file")
    ...
    try:
        ...
        raise TB2JError(f"{ucf_path} is not readable as a Vampire unit cell file: {error}") from error
    ...
    for line in mat_path.read_text().splitlines():
        ...
        if key.replace(" ", "") == "material:num-materials":
            try:
                ...
                raise TB2JError(f"{mat_path} has an unreadable num-materials: {error}") from error
    raise TB2JError(f"{mat_path} has no 'material:num-materials' line, so it is not a Vampire material file")
...
def read_tb2j_model(source_dir: Path) -> tuple[list[float], int, int]:
    ...
    for name in (UCF_NAME, MAT_NAME):
        if not (source_dir / name).is_file():
            raise TB2JError(f"{source_dir}/{name} is missing, so the TB2J run did not finish")
...
def build_input(mp_id: str, source_dir: Path, cell: list[float], args: Namespace) -> str:
...
def cli() -> Namespace:
    ...
    parser.add_argument("--tb2j-dir", type=Path, default=None, help="Directory holding the finished TB2J run (default: ./work/<mp_id>/tb2j)")
    parser.add_argument("-o", "--output-dir", type=Path, default=None, help="Directory to run Vampire in (default: ./work/<mp_id>/vampire)")
    parser.add_argument("--system-size", type=float, default=15.0, help="Edge of the cubic simulation box in nm, filled with whole unit cells (default: 15.0)")
    parser.add_argument("--temperature-min", type=float, default=0.0, help="Lowest temperature of the sweep in K (default: 0.0)")
    parser.add_argument("--temperature-max", type=float, default=1000.0, help="Highest temperature of the sweep in K (default: 1000.0)")
    parser.add_argument("--temperature-step", type=float, default=25.0, help="Temperature increment of the sweep in K (default: 25.0)")
    parser.add_argument("--equilibration-steps", type=int, default=2500, help="Time steps to equilibrate at each temperature before averaging (default: 2500)")
    parser.add_argument("--loop-steps", type=int, default=3000, help="Time steps to average over at each temperature (default: 3000)")
    parser.add_argument("--no-sbatch", action="store_true", help="Don't write a submit.sh sbatch script")
    parser.add_argument("--job-name", default=None, help="SLURM job name (default: <mp_id>-vampire)")
    ...
    parser.add_argument("--time", default="04:00:00", help="Wall-clock time limit, SLURM format (default: 04:00:00)")
    parser.add_argument("--vampire-binary", default="/lustre/work/ws/ws1/ka_qa7606-dft_pipeline/vampire/vampire-parallel", help="Vampire executable to run under srun (default: the vampire-parallel build in the DFT pipeline workspace)")
    parser.add_argument("--modules", default="compiler/intel/2024.2.1 mpi/impi/2021.13.1", help="Environment modules to load, matching the MPI the binary was linked against (default: compiler/intel/2024.2.1 mpi/impi/2021.13.1)")
...
def main():
    ...
    temperatures = int((args.temperature_max - args.temperature_min) // args.temperature_step) + 1

    print(f"Wrote the Vampire stage for {args.mp_id} to {output_dir}/")
    print(f"  model:         {source_dir}/ ({spins} spins per cell, {interactions} interactions)")
    print(f"  unit cell:     {' '.join(f'{length:.4f}' for length in cell)} A")
    print(f"  system:        {args.system_size} nm cube, {' x '.join(str(n) for n in cells)} cells, "
          f"{cells[0] * cells[1] * cells[2] * spins} spins")
    print(f"  sweep:         {args.temperature_min} to {args.temperature_max} K in steps of "
          f"{args.temperature_step} K ({temperatures} temperatures)")
    print(f"  per point:     {args.equilibration_steps} equilibration + {args.loop_steps} averaging steps")
